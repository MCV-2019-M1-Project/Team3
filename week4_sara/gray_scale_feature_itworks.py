...
for qs in range(0,nqs):
    for ds in range(0,nds):
        ...
        crit[qs,ds,:] = classif(dist, 130, 0.7)
#    print(qs)
...
#    print(i, np.max(crit[i,:,1]))
    ...
    if np.max(crit[i,:,1])<10:
        preds_flat.append(np.concatenate((np.array([-1]), np.argsort(-crit[i,:,1])[:9])).tolist())
    else:
        preds_flat.append(np.argsort(-crit[i,:,1])[:10].tolist())
...
mapat10 = metrics.mapk(gt_flat, preds_flat, k=10)
