...
def pres_rec_f1_2(GT, pred):
    ...
    for groundt in GT:
        ...
        if max_val == 0:
            max_val = 1
        recall[count] = metrics.recall_score(np.reshape(groundt[:, :, 0], sx * sy) / 255,
                                             np.reshape(pred[count], sx * sy) / max_val)
        precision[count] = metrics.precision_score(np.reshape(groundt[:, :, 0], sx * sy) / 255,
                                                   np.reshape(pred[count], sx * sy) / max_val)
        f1[count] = metrics.f1_score(np.reshape(groundt[:, :, 0], sx * sy) / 255,
                                     np.reshape(pred[count], sx * sy) / max_val)
        count = count + 1
